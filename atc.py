...
def data_consumer(queue, interval, space):
    ...
    while True:
        ...
        for i in planes:
            # print(i)
            if not space.plane.has_key(i['flight']):
                if (i['destination'] == "CTU") & (i['altitude'] < 6000) & (i['altitude'] > 1000) :
                    agent = QLearningAgent(baseAirplane(i['flightType'],i['flight'],i['registration'],i['depature'],i['destination'],None,None,
                        i['heading'],i['longitude'],i['latitude'],i['altitude'],i['groundSpeed'],i['squawk']),space)
                    space.plane[i['flight']] = agent
                    #store the state of all plane as another dict for convenience
                    space.planeLocation[i['flight']] = [agent.X,agent.Y,agent.Z]
        ...
        for _ in range(100000):
            ...
            action = random.choice(tempAgent.getLegalActions(startState))
            endState = tempAgent.getNextState(startState,action)
            reward = tempAgent.getReward(endState)
            lastExperience = (startState, action, endState, reward)
            tempAgent.update(*lastExperience)
